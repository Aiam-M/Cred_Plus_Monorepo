import { MapPin, Maximize2, Leaf, Star, User, Tag } from 'lucide-react';
  ...
  return (
    <div className="bg-white rounded-2xl border border-cred-gray-border shadow-sm card-hover flex flex-col">
      ...
      <div className="p-5 flex flex-col flex-1 gap-3">
        ...
        {/* AgroScore label */}
        <p className="flex items-center gap-1 text-xs font-medium text-cred-green-medium">
        ...
        <div className="space-y-1 text-sm text-gray-600">
          <div className="flex items-center gap-1">
            ...
            <span>{safra.associacao.nome}</span>
          </div>
          <div className="flex items-center gap-1">
            <User className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" />
            <span>Produtor: {safra.produtor.nome}</span>
          </div>
          <div className="flex items-center gap-1">
        ...
        <div className="flex flex-wrap gap-1.5">
          {safra.plantacoes.map((p, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1 px-2.5 py-1 bg-cred-beige text-cred-green-dark text-xs rounded-lg font-medium"
            >
              <Tag className="w-3 h-3 flex-shrink-0" />
              {TIPO_LABEL[p.tipo] ?? p.tipo}: {p.quantidade} {p.unidade}
            </span>
          ))}
        </div>
